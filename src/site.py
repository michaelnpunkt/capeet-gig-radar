...
<nav class="status-bar" aria-label="Datenstand und Feeds"><span id="header-metadata">Datenstand wird geladen …</span><span title="Die öffentliche Capeet-Seite wird einmal pro Tag abgefragt; zwischendurch bleibt der zuletzt erfolgreiche Stand online.">Abruf: 1× täglich</span><a href="changes.html">Changelog</a><a href="https://www.capeet.com/gigs_list.html" rel="noopener noreferrer">Capeet Original</a><a href="feed.xml">Gesamt-RSS</a><details class="feed-menu"><summary>Bundesland-RSS</summary><div><a href="feeds/neu-burgenland.xml">Burgenland</a><a href="feeds/neu-karnten.xml">Kärnten</a><a href="feeds/neu-niederosterreich.xml">Niederösterreich</a><a href="feeds/neu-oberosterreich.xml">Oberösterreich</a><a href="feeds/neu-salzburg.xml">Salzburg</a><a href="feeds/neu-steiermark.xml">Steiermark</a><a href="feeds/neu-tirol.xml">Tirol</a><a href="feeds/neu-vorarlberg.xml">Vorarlberg</a><a href="feeds/neu-wien.xml">Wien</a></div></details><span id="header-feeds" class="header-feeds" aria-label="RSS der ausgewählten Bundesländer"></span></nav></header>
...
<a class="back-top" href="#top" aria-label="Zurück zum Seitenanfang">↑ Nach oben</a><footer><p id="metadata">Datenstand wird geladen …</p><p class="feeds"><a href="changes.html">Alle Änderungen ansehen</a> · <a href="feed.xml">RSS: alle Neuigkeiten</a></p><p id="selected-feeds" class="feeds" aria-label="RSS-Feeds der ausgewählten Bundesländer"></p>
...
function save(){const state={q:search.value,states:[...selected('#states input')],genres:[...selected('#genres input')],month:month.value,days:days.value,past:past.checked,cancelled:cancelled.checked,changes:changes.checked,sort:sort.value,view:viewMode};localStorage.setItem('capeet-filters',JSON.stringify(state));const params=new URLSearchParams();if(state.q)params.set('q',state.q);if(state.states.length!==1||state.states[0]!=='Wien')params.set('states',state.states.join(','));if(state.genres.length)params.set('genres',state.genres.join(','));if(state.month)params.set('month',state.month);if(state.days)params.set('days',state.days);if(!state.past)params.set('past','0');if(state.cancelled)params.set('cancelled','1');if(state.changes)params.set('changed','1');if(state.sort!=='discovered-desc')params.set('sort',state.sort);if(state.view==='list')params.set('view','list');history.replaceState(null,'',`${location.pathname}${params.size?'?'+params:''}`)}
...
async function start(){try{const [response,statusResponse]=await Promise.all([fetch('data/gigs.json'),fetch('data/status.json')]);if(!response.ok||!statusResponse.ok)throw new Error('HTTP');const payload=await response.json(),statusPayload=await statusResponse.json(),formatter=new Intl.DateTimeFormat('de-AT',{dateStyle:'medium',timeStyle:'short'});events=payload.events;STATES.forEach(value=>checkbox(byId('states'),value,value==='Wien'));[...new Set(events.map(item=>item.genre.family))].sort((a,b)=>a.localeCompare(b,'de')).forEach(value=>checkbox(byId('genres'),value,false));[...new Set(events.map(item=>item.event_date.slice(0,7)))].sort().forEach(value=>{const option=node('option','',new Intl.DateTimeFormat('de-AT',{month:'long',year:'numeric'}).format(new Date(`${value}-01T12:00:00`)));option.value=value;month.append(option)});restore();const checked=`Zuletzt geprüft: ${formatter.format(new Date(statusPayload.checked_at))}`,changed=`Daten geändert: ${formatter.format(new Date(statusPayload.changed_at))}`;byId('header-metadata').textContent=`${checked} · ${events.length} Gigs`;byId('metadata').textContent=`${checked} · ${changed} · ${events.length} Gigs`;render()}catch{count.textContent='Die Konzertdaten konnten nicht geladen werden.'}}
document.addEventListener('input',event=>{if(event.target.closest('.controls'))render()});document.querySelectorAll('[data-states]').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('#states input').forEach(item=>item.checked=button.dataset.states==='all');render()}));viewButtons.forEach(button=>button.addEventListener('click',()=>{setView(button.dataset.view);save()}));byId('reset').addEventListener('click',()=>{search.value='';month.value='';days.value='';past.checked=true;cancelled.checked=false;changes.checked=false;sort.value='discovered-desc';setView('grid');document.querySelectorAll('#states input').forEach(item=>item.checked=item.value==='Wien');document.querySelectorAll('#genres input').forEach(item=>item.checked=false);render()});start();
"""
